...
import { NumberRepository } from '../../data/Settings/NumberRepository'
import { NumberEntity } from '../../data/Settings/NumberEntity'

const validationModules = await Extensions.loadExtensionModules('codbex-number-generator-Settings-Number', ['validate']);
...
class NumberController {
    ...
    @Inject('NumberRepository')
    private readonly repository!: NumberRepository;

    @Get('/')
    @Documentation('Get All Number')
    public getAll(_: any, ctx: any): NumberEntity[] {
        try {
    ...
    public create(entity: NumberEntity): NumberEntity {
        try {
    ...
    public search(filter: any): NumberEntity[] {
        try {
            this.checkPermissions('read');
    ...
    public getById(_: any, ctx: any): NumberEntity {
        try {
            this.checkPermissions('read');
    ...
    public update(entity: NumberEntity, ctx: any): NumberEntity {
        try {
            this.checkPermissions('write');
            const id = parseInt(ctx.pathParameters.id);
            entity.Id = id;
    ...
    private checkPermissions(operationType: string) {
        if (operationType === 'read' && !(user.isInRole('codbex-number-generator.Numbers.NumberReadOnly') || user.isInRole('codbex-number-generator.Numbers.NumberFullAccess'))) {
            throw new ForbiddenError();
        }
        if (operationType === 'write' && !user.isInRole('codbex-number-generator.Numbers.NumberFullAccess')) {
            throw new ForbiddenError();
        }
    }
